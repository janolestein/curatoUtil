...
async function addNav() {
  ...
  const plusBlock = Array.from(allTechnicalNames).find(
    (span) => span.innerText === "bild-plus-artikel",
  );
  const regioBlock = Array.from(allTechnicalNames).find(
    (span) => span.innerText === "regional",
  );
  const geldBlock = Array.from(allTechnicalNames).find(
